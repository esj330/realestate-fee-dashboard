...
def create_plotly_spatial_scatter(df: pd.DataFrame, value_col: str):
    ...
    fig = px.scatter(
        ...
        y='lat',
        color=display_col,
        color_continuous_scale=PLOTLY_RED_SCALE,
        hover_data=['단지명', '거래유형', '건물유형', '거래연월'],
        title=f'공간분포 산점도 ({color_label})',
    )
    fig.update_layout(template='plotly_white', xaxis_title='경도', yaxis_title='위도', coloraxis_colorbar_title=color_label)
    return fig
...
heat_gradient_name = st.selectbox('열지도 색상 강도', list(HEATMAP_GRADIENTS.keys()), index=0)
